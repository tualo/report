...
DELIMITER //
CREATE OR REPLACE FUNCTION `getArticleTaxRate`(
        in_steuergruppe varchar(20),
        in_bookingdate date,
        in_article varchar(255)
    ) RETURNS fixed(15, 6) READS SQL DATA COMMENT '' BEGIN
DECLARE fld varchar(10);
DECLARE result varchar(10);
SELECT ifnull(feld, '') INTO fld
FROM steuergruppen
where steuergruppe = in_steuergruppe;
select ifnull(JSON_VALUE(json_row, concat('$.steuer', fld)), 19) INTO result
from view_readtable_bfkonten_jsonrow
    join bfkonten_zuordnung on view_readtable_bfkonten_jsonrow.id = bfkonten_zuordnung.konto_id
    and view_readtable_bfkonten_jsonrow.gueltig = bfkonten_zuordnung.gueltig
    and bfkonten_zuordnung.gruppe = in_article
WHERE in_bookingdate between view_readtable_bfkonten_jsonrow.gueltig_von and view_readtable_bfkonten_jsonrow.gueltig;
return result;
END //
CREATE OR REPLACE FUNCTION `getArticleAccountNumber`(
        in_steuergruppe varchar(20),
        in_bookingdate date,
        in_article varchar(255)
    ) RETURNS varchar(10) READS SQL DATA COMMENT '' BEGIN
DECLARE fld varchar(10);
DECLARE result varchar(10);
SELECT ifnull(feld, '') INTO fld
FROM steuergruppen
where steuergruppe = in_steuergruppe;
select JSON_VALUE(json_row, concat('$.konto', fld)) INTO result
from view_readtable_bfkonten_jsonrow
    join bfkonten_zuordnung on view_readtable_bfkonten_jsonrow.id = bfkonten_zuordnung.konto_id
    and view_readtable_bfkonten_jsonrow.gueltig = bfkonten_zuordnung.gueltig
    and bfkonten_zuordnung.gruppe = in_article
...
DELIMITER //
CREATE OR REPLACE PROCEDURE checkReportRequirements(in reporttype varchar(20), in in_json JSON) BEGIN IF NOT JSON_VALID(in_json) THEN SIGNAL SQLSTATE '45000'
...
END IF;
END // -- call getReport('rechnung',200901667,@res);
-- select @res;
...
END // -- SOURCE FILE: ./src/000.RPTJSON.proc.getPositions.sql 
DELIMITER //
...
DEALLOCATE PREPARE stmt;
SET result = @positions;
END // -- SOURCE FILE: ./src/000.RPTJSON.proc.getReductions.sql 
DELIMITER;
...
INSERT IGNORE INTO blgreductions_translations (column_name, json_attribute_name, is_required)
values ('belegnummer', 'reportnr', 1);
DELIMITER // DROP PROCEDURE IF EXISTS getReportReductions // CREATE PROCEDURE `getReportReductions`(
    in reporttype varchar(20),
    in in_id bigint,
    out result JSON
) READS SQL DATA COMMENT '' BEGIN
...
DEALLOCATE PREPARE stmt;
SET result = @positions;
END // -- SOURCE FILE: ./src/000.RPTJSON.proc.getReportLocks.sql 
DELIMITER // DROP PROCEDURE IF EXISTS getReportLocks // CREATE PROCEDURE `getReportLocks`(
    in reporttype varchar(20),
    in in_id bigint,
    out result JSON
) READS SQL DATA COMMENT '' BEGIN
...
END // -- SOURCE FILE: ./src/000.RPTJSON.proc.setReport.sql 
DELIMITER //
CREATE OR REPLACE PROCEDURE setReportOLD(
        in reporttype varchar(20),
        in in_json JSON,
        out out_json JSON
    ) BEGIN
...
SET @positionid := 0;
SET @SQL := CONCAT(
        'SELECT ifnull(max(id),0)+1 i INTO @positionid FROM blg_pos_',
        reporttype
    );
PREPARE stmt
FROM @SQL;
...
ELSEIF @newrptnr IS NOT NULL
AND JSON_EXISTS(position, '$.id') = 1 THEN
...
SET @USQL = concat(
        'update blg_pos_',
        JSON_VALUE(xref, '$.tabellenzusatz'),
        ' set zid=\'',
        JSON_VALUE(position, '$.id'),
        '\',zzusatz=\'',
        reporttype,
        '\' where id=',
        JSON_VALUE(xref, '$.posid'),
        ' '
    );
...
DEALLOCATE PREPARE stmt;
SET @USQL = concat(
        'insert ignore into blg_ueb_',
        reporttype,
        ' (id,zid,tabellenzusatz) values (',
        JSON_VALUE(position, '$.reportnr'),
        ',',
        JSON_VALUE(xref, '$.belegnummer'),
        ',\'',
        JSON_VALUE(xref, '$.tabellenzusatz'),
        '\')'
    );
...
SET @paymentid := 0;
SET @SQL := CONCAT(
        'SELECT ifnull(max(id),0)+1 i INTO @paymentid FROM blg_pay_',
        reporttype
    );
-- call debug_message(@SQL );
PREPARE stmt
...
SET @USQL = concat(
        'insert into blg_txt_',
        reporttype,
        ' (`id`,`typ`,`text`) values (',
        JSON_VALUE(position, '$.reportnr'),
        ',\'',
        JSON_VALUE(textval, '$.type'),
        '\',\'',
        JSON_VALUE(textval, '$.text'),
...
END IF;
SET @SQL = concat(
        'SELECT round(sum(steuer),2) s, round(sum(netto),2) n, round(sum(brutto),2) b INTO  @use_s, @use_n, @use_b FROM blg_pos_',
        reporttype,
        ' WHERE beleg = ',
        JSON_VALUE(position, '$.reportnr')
    );
...
SET @SQL = concat(
        'UPDATE  blg_hdr_',
        reporttype,
        '   SET    netto = @use_n,   brutto = @use_b,   steuer = @use_b-@use_n  WHERE     id = ',
        JSON_VALUE(position, '$.reportnr')
    );
PREPARE stmt
FROM @SQL;
...
-- call debug_message(reporttype);
IF lower(reporttype) <> 'wb' THEN -- call setWarehouseReport(@tempout,@tempout2);
SET @tempout = JSON_SET(@tempout, '$.__wb', @tempout2);
call debug_message(
    concat(
        '_in_warehouse ',
        JSON_EXISTS(@tempout, '$._in_warehouse')
    )
);
/*
...
END // -- SOURCE FILE: ./src/000.RPTJSON.proc.setReportDefaults.sql 
DELIMITER //
CREATE OR REPLACE PROCEDURE setReportDefaults(
        in reporttype varchar(20),
        in in_json JSON,
        out out_json JSON
    ) BEGIN
...
IF JSON_EXISTS(in_json, '$.id') = 1
OR json_value(in_json, '$.id') > 0 THEN
SET @old_rn = null;
SET @SQL := CONCAT(
...
SET @SQL = concat(
        '
        insert into blg_hdr_',
        reporttype,
        ' (id,datum,faelligam,login) values (@newrptnr,current_date,current_date,getSessionUser())
        '
    );
PREPARE stmt
FROM @SQL;
...
END // -- SOURCE FILE: ./src/000.RPTJSON.proc.setReportHeader.sql 
DELIMITER // CREATE TABLE IF NOT EXISTS blghdr_translations (
...
INSERT IGNORE INTO blghdr_translations (column_name, json_attribute_name, is_required)
values ('offen', 'open', 0) //
CREATE OR REPLACE PROCEDURE setReportHeader(in reporttype varchar(20), in in_json JSON) BEGIN
...
SET done = TRUE;
OPEN c1;
read_loop: LOOP FETCH c1 INTO column_name,
json_attribute_name,
is_required;
If done THEN LEAVE read_loop;
END IF;
IF NOT JSON_EXISTS(in_json, concat('$.', json_attribute_name)) THEN IF is_required = 1 THEN
...
IF JSON_TYPE(
    JSON_EXTRACT(in_json, concat('$.', json_attribute_name))
) <> 'NULL' THEN
set v := QUOTE(v);
ELSE
set v := 'null';
...
DEALLOCATE PREPARE stmt;
SET @SQL := concat(
        'INSERT INTO blg_bkr_',
        reporttype,
        ' (id,buchungskreis_id) values (',
        JSON_VALUE(in_json, '$.id'),
        ',',
        QUOTE(JSON_VALUE(in_json, '$.companycode')),
        ') ON DUPLICATE KEY UPDATE buchungskreis_id=values(buchungskreis_id) '
    );
PREPARE stmt
FROM @SQL;
execute stmt;
DEALLOCATE PREPARE stmt;
SELECT ds.table_name rt,
    bezug_id,
    bezug_kst INTO @base_table,
    @readtable_bezug_id,
    @readtable_bezug_kst
FROM blg_config
    join ds on ds.table_name = blg_config.adress_bezug
...
SET @SQL := concat(
        ...
        '_',
        reporttype,
        ' (id,kundennummer,kostenstelle) values (',
        JSON_VALUE(in_json, '$.id'),
        ',',
        QUOTE(JSON_VALUE(in_json, '$.referencenr')),
        ',',
        QUOTE(JSON_VALUE(in_json, '$.costcenter')),
        ') ON DUPLICATE KEY UPDATE kundennummer=values(kundennummer),kostenstelle=values(kostenstelle) '
    );
PREPARE stmt
FROM @SQL;
execute stmt;
...
SET @adr := '';
SELECT if(
        ifnull(ds.read_table, '') = '',
        ds.table_name,
        ds.read_table
    ) rt,
    bezug_id,
    bezug_kst INTO @readtable_name,
    @readtable_bezug_id,
    @readtable_bezug_kst
FROM blg_config
    join ds on ds.table_name = blg_config.adress_bezug
    join ds_column on lower(ds_column.column_name) = 'report_address'
...
END IF;
SET @adr := replace(
        replace(@adr, concat(char(10), char(10)), char(10)),
        concat(char(10), char(10)),
        char(10)
    );
SET @SQL := concat(
        'INSERT INTO blg_adr_',
        reporttype,
        ' (id,adresse) values (',
        JSON_VALUE(in_json, '$.id'),
        ',@adr) ON DUPLICATE KEY UPDATE adresse=values(adresse) '
    );
call debug_message(@SQL);
PREPARE stmt
FROM @SQL;
...
END // -- SOURCE FILE: ./src/000.RPTJSON.proc.setReportN.sql 
DELIMITER // DELIMITER //
CREATE OR REPLACE PROCEDURE setReportDefaultsN(
        in reporttype varchar(20),
        in in_json JSON,
        out out_json JSON
    ) BEGIN
...
SET in_json = JSON_SET(
        ...
        JSON_VALUE(in_json, concat('$.', rec.json_attribute_name))
    );
END IF;
END FOR;
...
SET @request = JSON_OBJECT(
        'tablename',
        concat('blg_hdr_', reporttype),
        'data',
        JSON_ARRAY(JSON_MERGE(in_json, '{}')),
        'type',
        'insert',
        'update',
        1 = 1
...
SET @MSG = concat(
        'Error while inserting into blg_hdr_',
        reporttype,
        ' : ',
...
DELIMITER //
CREATE OR REPLACE PROCEDURE setReportPosition(in reporttype varchar(20), in in_json JSON) BEGIN
DECLARE flds LONGTEXT default '';
DECLARE vals LONGTEXT default '';
DECLARE upda LONGTEXT default '';
DECLARE v LONGTEXT default '';
...
END IF;
SELECT @tempout INTO out_json;
END // -- SOURCE FILE: ./src/000.TBL.wb_artikel_combinations.sql 
DELIMITER;
...
DELIMITER // create view if not exists view_readtable_blg_config_buchungskreis as
select '0000' buchungskreis_id,
    id blg_id,
    belegnummer_von von,
    belegnummer_bis bis
from blg_config // -- create unique index idx_blg_pos_wb_fid_fzusatz on  blg_pos_wb(fid,fzusatz)
    -- SOURCE FILE: ./src/calculateGLD.sql 
    DELIMITER //
CREATE OR REPLACE FUNCTION `calculateGLD`(
        in_booking_circle varchar(20),
        in_report_nr bigint
    ) RETURNS varchar(10) READS SQL DATA COMMENT '' BEGIN
DECLARE fld varchar(10);
DECLARE result varchar(10);
SELECT artikel,
    (wert_alt + wert_neu /(menge_alt + menge_neu)) gld INTO @nonce,
    result
from (
        SELECT blg_pos_wb.artikel,
        ...
        FROM blg_pos_wb
            join (
                select artikel,
                    blg_bkr_wb.buchungskreis_id
                from blg_pos_wb
                    join blg_bkr_wb on blg_pos_wb.beleg = blg_bkr_wb.id
                where blg_pos_wb.beleg = in_report_nr
            ) nblg on blg_pos_wb.artikel = nblg.artikel
            and artikel.buchungskreis_id = nblg.buchungskreis_id
            left join bewertungspreis on blg_pos_wb.artikel <> bewertungspreis.gruppe
            and bewertungspreis.typ = 'GLD'
        WHERE blg_pos_wb.id < (
                select min(id)
                from blg_pos_wb
                where beleg = in_report_nr
            )
    ...
    join (
        ...
        FROM (
            ...
            and bewertungspreis.typ = 'GLD'
        GROUP BY blg_pos_wb.artikel
    ) neu on alt.artikel = neu.artikel;
return result;
END //