Ext.define('Tualo.report.lazy.controller.ReportPay', {
  ...
  initializeReport: function () {
    let me = this,
      ...
      minformpanel = view.getComponent('minform'),
    ...
    if (Ext.isEmpty(Ext.ClassManager.getByAlias('widget.dsform_view_editor_blg_min_' + view.type))) {
      // Ext.toast('Das Minderungs-Formular ist nicht konfiguriert', 2000);
      console.info('Das Minderungs-Formular ist nicht konfiguriert. Bitte den Datenstamm view_editor_blg_min_' + view.type + ' anpassen.');
      // return;
    } else {
      me.minform = Ext.create('widget.dsform_view_editor_blg_min_' + view.type, {
        title: null,
        buttons: [
          {
            text: 'Schliessen',
          ...
          {
            text: 'Mindern',
            handler: 'addReduction'
          }
        ]
      });

      minformpanel.removeAll();
      minformpanel.add(me.minform);
  ...
  */
    ...
    let data = await fetch('./reducereport/' + view.type + '/' + view.reportnumber, { method: 'PUT', body: JSON.stringify(o) }).then((response) => { return response.json() });
    if (data.success == true) {
    }
    me.list.getStore().load();
    me.loadReport();
